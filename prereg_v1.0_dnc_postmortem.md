...
| 2026-05-23 | **ANES outcome mis-codings in `data_prep.py` v2 — both corrected.** (a) **`single_payer` outcome was using V241247** which is the **placement of Donald Trump** on the 7pt gov-vs-private medical insurance scale, NOT the respondent's own view. Correct self-placement is **V241245** (1=Gov insurance, 7=Private insurance; "99=haven't thought" dropped, n=4218 usable). First fit on V241247 is meaningless and DELETED from `data/processed/fits/`. (b) **`israel` outcome was using V241404** which is "Favor/Oppose U.S. giving humanitarian aid to Palestinians in Gaza" (1=Favor, 2=Oppose, 3=Neither), NOT a direct Israel-position item. Variable is correctly captured but the outcome was mis-labeled. Renamed to **`gaza_aid_pal`** in fit outputs (existing fit retained — content is correct, label was wrong). Added two direct Israel/Gaza items: **`israel_military`** = V241401 (favor/oppose US military aid to Israel) and **`gaza_protests`** = V241410 (approve/disapprove campus protests against war in Gaza). | Codebook re-verification at result-read time 2026-05-23. Discovered after batch A produced sensible-looking but mis-interpreted output. Caught BEFORE building variance-decomposition table or writing v1 result. Fixes filed before re-running. The Israel/Palestine cluster on ANES is now operationalized at three distinct items (aid-to-Palestinians, military-aid-to-Israel, Gaza-protests-approval) instead of one ambiguous proxy; per pre-reg §3 this strengthens rather than weakens Issue 1 coverage on ANES. |
